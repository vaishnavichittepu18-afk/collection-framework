...
EnumSet<Day> enumSet = EnumSet.of(Day.MON);

/*About:
-Used only with enum types
-Stores unique enum constants
...
EnumSet.all0f (Day.class);           //All enum constants
EnumSet.noneOf(Day.class);           //Empty EnumSet
EnumSet.of(Day. MON, Day. TUE);      //Creates EnumSet with specified values
EnumSet.range (Day. MON, Day. FRI);  //values in range Enum
EnumSet.complementOf(enumSet);       //Opposite enum values
EnumSet.copyOf(enumSet);             //Creates copy
